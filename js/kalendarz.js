...
function hideKal()
{
	...
	if(CzyZmianaTerminu == 1 && document.getElementById("ZmianaTerminu")){
        ...
        if(CzyZmianaTerminu == 1 && $("#data_wplywu") && $("#termin_przewoznika")){
            ObliczTerminPrzewoznika();
        }
}
...
function rysujKal()
{
	...
	kaltxt += '</select>&nbsp;<select name="year" class="Kallista2" onChange="setData()">';
        obecny_rok = new Date();
	ObecnyRok = obecny_rok.getFullYear();
	for(i=(ObecnyRok-wstecz);i<=(ObecnyRok+wprzod);i++)
	{
		if(i==Rok)
	...
	kaltxt += '<tr class=Kaldnityg><td width=30>Nd</td><td width=30>Pn</td><td width=30>Wt</td><td width=30>Śr</td>';
