...
class Formularz {
	...
	function ZwrocNazweMapyPola($Nazwa){
        ...
        function dieMotherfuckerDie($data) {
            echo '<pre>';
            var_dump($data);
            echo '---';
            var_dump($this->Kolejnosc);
            echo '---';
            var_dump($this->MapaNazw);
            echo '---';
            var_dump($this->Pola);
            echo '---';
            echo '</pre>';
		...
		for ($i = 0; $i < count($this->Kolejnosc); $i++) {
			...
			if (isset($Wartosci[$NazwaPola])) {
				...
				$this->WyswietlPole($this->Kolejnosc[$i]);
                            //$this->dieMotherfuckerDie($this->Kolejnosc[$i]);
			}
		}
		echo('</table></form>');
	}
	...
	function PolaFormularzaNiestandardowe($Typ = null, $Nazwa, $Wartosc, $AtrybutyDodatkowe, $pole_id){
		switch($Typ){
                    ...
                    case 'wyslij_raport':
                        ...
                        break;
                    case 'wybierz_jezyk':
                        include(SCIEZKA_SZABLONOW."forms/wybierz_jezyk.tpl.php");
                        break;
