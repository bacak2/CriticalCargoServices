...
class PlatnosciLotnicze extends ModulBazowy {
        public $Klienci;
        public $Przewoznicy;
				...
				if (isset($_SESSION['Filtry'][$Pole])) {
                                    ...
                                    if($this->Filtry[$i]['typ'] == "lista"){
                                        ...
                                        if($Pole == "nip_search"){
                                            ...
                                            $Clients[] = -1;
                                            $Where .= ($Where != '' ? ' AND ' : '').($AliasTabeli ? "$AliasTabeli." : '')."id_przewoznik IN(".implode(",",$Clients).")";
                                        }else{
	...
	function PobierzListeElementow($Filtry = array()) {
        ...
        function ObrobkaDanychLista($Elementy){
            foreach($Elementy as $Idx => $Element){
                $Elementy[$Idx]['faktury'] = $this->Baza->GetRows("SELECT * FROM orderplus_air_orders_faktury WHERE id_zlecenia = '{$Element[$this->PoleID]}'");
                $Elementy[$Idx]['koszty'] = $this->Baza->GetRows("SELECT * FROM orderplus_air_orders_koszty WHERE id_zlecenie = '{$Element[$this->PoleID]}'");
            }
            return $Elementy;
        }
        ...
        function ShowRecords($Faktury, $Koszty, $i){
            ...
            print ("<td$Styl>");
                if(isset($Faktury[$i])){
                    $Pozycje = mysql_query("SELECT * FROM orderplus_air_orders_faktury_pozycje WHERE id_faktury = '{$Faktury[$i]['id_faktury']}'");
                    $PosMany = 0;
                    $PosManyWaluta = 0;
        ...
        function AkcjaEdycja($ID){
            if($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['nowy'] == "nowy"){
                if($this->Zapisz($ID)){
                    ...
                    include(SCIEZKA_SZABLONOW."forms/platnosci-lotnicze-form.tpl.php");
                }
            }else{
                $zlecenie = $this->PobierzDaneElementu($ID);
                $Faktury = $this->Baza->GetRows("SELECT * FROM orderplus_air_orders_faktury WHERE id_zlecenia = '$ID'");
                $Koszty = $this->Baza->GetRows("SELECT * FROM orderplus_air_orders_koszty WHERE id_zlecenie = '$ID'");
                $StawkiVat = Usefull::GetStawkiVat();
                $Statusy = Usefull::StatusyPlatnosciAirSea();
                $StatusyKlient = Usefull::StatusyPlatnosciKlient();
                include(SCIEZKA_SZABLONOW."forms/platnosci-lotnicze-form.tpl.php");
        ...
        function Zapisz($ID){
            ...
            foreach($Fak as $FID => $Faktura){
                $Zapytanie = $this->Baza->PrepareUpdate("orderplus_air_orders_faktury", $Faktura, array("id_faktury" => $FID));
            ...
            foreach($Costs as $CID => $Koszt){
                ...
                $Zap2 = $this->Baza->PrepareUpdate("orderplus_air_orders_koszty", $Koszt, array("id_koszt" => $CID));
