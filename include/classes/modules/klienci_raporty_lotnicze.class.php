<?php
/**
 * Moduł klienci - raporty lotnicze
 * 
...
class KlienciRaportyLotnicze extends ModulBazowy {
        public $Emaile;
	...
	function __construct(&$Baza, $Uzytkownik, $Parametr, $Sciezka) {
            ...
            $this->Tabela = 'orderplus_air_klient_raport';
            $this->TabelaZlecenia = 'orderplus_air_klient_raport_zlecenie';
            ...
            $this->HashPrefix = "AirRaportKlient";
	}
	...
	function &GenerujFormularz() {
		...
		$Akcje = array();
                ...
                $Akcje[] = array('img' => "podglad_button", 'title' => "Podgląd", "akcja_link" => "raports/raport_lotniczy.php?check={$Dane["hash"]}");
	...
	function PobierzListeElementow($Filtry = array()) {
        ...
        function PobierzZlecenia($ID){
            return $this->Baza->GetResultAsArray("SELECT id_zlecenie, numer_zlecenia as numer, zlecenie_status as status FROM $this->TabelaZlecenia tz JOIN  orderplus_air_orders z ON(z.id_zlecenie = tz.zlecenie_id) WHERE $this->PoleID = '$ID'", "id_zlecenie");
        }
        ...
        function AkcjaDodawanie() {
                ...
                if(isset($_GET['id']) && is_numeric($_GET['id'])){
                    $Zlecenie = $this->Baza->GetData("SELECT * FROM orderplus_air_orders WHERE id_zlecenie = '{$_GET['id']}'");
                ...
                if(isset($_POST['OrdersIDs']) && $_POST['OrdersIDs'] != ""){
                    ...
                    foreach($ids as $ZlecID){
                        $ZlecDane = $this->Baza->GetData("SELECT * FROM orderplus_air_orders WHERE id_zlecenie = '$ZlecID'");
                        if($ClientID == false && $ZlecDane['id_klient_shipper'] > 0){
			...
			if (in_array($OpcjaFormularza, array('zapisz', 'zapisz_zmiany', 'dodaj_nowe')) || $DodajOdRazu){
				...
				if($this->SprawdzDane($Wartosci) && $this->SprawdzPolaWymagane($PolaWymagane, $Wartosci) && $this->SprawdzPolaNieDublujace($PolaZdublowane, $Wartosci)){
					...
					Usefull::ShowKomunikatError('<b>'.$this->Error.'</b>');
                            ...
                            $Wartosci = $Formularz->ZwrocWartosciPol($_POST);
                            $this->ZleceniaKlienta = $this->GetZlecenia();
        ...
        function GetEmailTresc($Hash){
            ...
            $tresc_maila .= "<a href='http://plus.meppeurope.com/raports/raport_lotniczy.php?check=$Hash'>http://plus.meppeurope.com/raports/raport_lotniczy.php?check=$Hash</a><br /><br />\n";
        ...
        function AkcjaDrukuj($ID){
            ...
            if($Raport){
                $Zlecenia = $this->Baza->GetResultAsArray("SELECT zl.*, pzl.zlecenie_status FROM $this->TabelaZlecenia pzl
                                                            JOIN  orderplus_air_orders zl ON(zl.id_zlecenie = pzl.zlecenie_id)
                ...
                include(SCIEZKA_SZABLONOW."druki/raport_lotniczy_klient.tpl.php");
            }
        }
        ...
        function GetZlecenia(){
            return $this->Baza->GetOptions("SELECT id_zlecenie, numer_zlecenia FROM orderplus_air_orders WHERE ((ost_korekta = 1) OR (ost_korekta = 0 AND korekta = 0)) ORDER BY data_zlecenia DESC");
        }
}
?>
