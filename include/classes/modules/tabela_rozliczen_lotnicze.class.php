...
class TabelaRozliczenLotnicze extends ModulBazowy {
	...
	function PobierzListeElementow($Filtry = array(), $XLS = false) {
                ...
                $Wynik = array(
                    ...
                    "mawb" => array("naglowek" => "MAWB No"),
                    "hawb" => array("naglowek" => "HAWB No")
		);
		...
		$Akcje = array();
                $Akcje[] = array('img' => "faktura_button", 'title' => "Fakturuj", "akcja_link" => "?modul=faktury_lotnicze&akcja=dodawanie&aoid={$Dane['id_zlecenie']}", "faktury" => $this->Faktury);
        ...
        function AkcjaPopraw($ID){
            $ZlecenieLotnicze = new AirOrders($this->Baza, $this->Uzytkownik, $this->Parametr, $this->Sciezka);
            ...
            $DefaultValues['Koszty'] = $this->Baza->GetRows("SELECT * FROM orderplus_air_orders_koszty WHERE id_zlecenie = '$ID'");
            if(!$DefaultValues['Koszty']){
                $DefaultValues["Koszty"] = array();
            }
            if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['Air'])){
                if($ZlecenieLotnicze->Zapisz($Type, $Skrot, $DefaultValues)){
                    $this->ShowOK();
                }else{
                    $Values = $_POST['Air'];
                ...
                $Values = $ZlecenieLotnicze->PobierzKontenery($Values, $ID);
                include(SCIEZKA_SZABLONOW."forms/air-order-form.tpl.php");
            }
        }
        ...
        function ShowRecord($Element, $Nazwa, $Styl){
            ...
            if($Nazwa == "przychod"){
                echo "<td$Styl>\n";
                  $this->Faktury = array();
                  $Faktury = $this->Baza->GetResultAsArray("SELECT id_faktury, id_waluty, kurs, numer FROM orderplus_air_orders_faktury WHERE id_zlecenia = '{$Element[$this->PoleID]}'", "id_faktury");
                  $StawkaKlientPLN = 0;
                  if(count($Faktury) > 0){
                      foreach($Faktury as $FID => $DaneFak){
                        $this->Faktury[$FID] = $DaneFak['numer'];
                        $Fakturki[] = $FID;
                        $this->Baza->Query("SELECT * FROM orderplus_air_orders_faktury_pozycje WHERE id_faktury = '$FID'");
                        $PosMany = 0;
                ...
                $RamkaTabela = "#467AAC";
                  ...
                  print ("<td$Styl>");
                    ...
                    $this->Baza->Query("SELECT * FROM orderplus_air_orders_koszty WHERE id_zlecenie = '{$Element[$this->PoleID]}'");
                    $KL = false;
                            ...
                            if(!isset($Actions['hidden']) || !$Actions['hidden']){
                                if(isset($Actions['faktury'])){
                                    ...
                                    foreach($Actions['faktury'] as $IDF => $Numer){
                                        echo "<nobr><a href='javascript:ShowOptions(\"#faktury_{$Element[$this->PoleID]}\", $IDF, \"faktura-lotnicza\")' class=\"akcja\">$Numer</a></nobr><br />";
                                    }
                                }else{
            ...
            echo "<div style='clear: both'></div>\n";
                if($this->Uzytkownik->IsAdmin()){
                    ...
                    <div style='margin-bottom: 10px; color: #bcce00; padding: 10px; width: 98%; float: left; border-bottom: 1px solid #bbce00; border-top: 1px solid #bbce00;'>
                        <a href='tabela_rozliczen_lotnicze_xls.php' target='_blank' class='form-button' style="margin-left: 60px;">Generuj plik XLS</a>
                    </div>
                    <?php
                }
            }else{
        ...
        function ObrobkaDanychLista($Elementy) {
            $this->Sumowanie['numer_zlecenia'] = "<form name='orders' id='orders' target='_blank' action='' method='post'><input type='hidden' id='orders_ids' name='OrdersIDs' value='' /><input type='button' value='raport' class='form-button' onclick='RaportOrdersLotnicze();' /></form>";
            return $Elementy;
        }
