...
class ZleceniaLotnicze extends ModulBazowy {
    public $SOIE;
    public $SOIEK;
    public $Emaile;
    function __construct(&$Baza, $Uzytkownik, $Parametr, $Sciezka) {
            parent::__construct($Baza, $Uzytkownik, $Parametr, $Sciezka);
            $this->Tabela = 'orderplus_air_orders_zlecenia';
            $this->PoleID = 'zlecenie_ao_id';
            $this->PoleNazwy = 'numer_zlecenia';
            $this->Nazwa = 'Zlecenie';
            $this->CzySaOpcjeWarunkowe = true; 
            $this->SOIE = $this->Baza->GetOptions("SELECT id_zlecenie, numer_zlecenia FROM orderplus_air_orders WHERE ".($this->Uzytkownik->CheckNoOddzial() ? "id_oddzial = '{$_SESSION['id_oddzial']}' AND" : "")." data_zlecenia >= '{$_SESSION['okresStart']}-01' AND data_zlecenia <= '{$_SESSION['okresEnd']}-31' ORDER BY numer_zlecenia_krotki ASC, numer_zlecenia ASC");
            $this->SOIEK = array_keys($this->SOIE); 
            $this->Filtry[] = array("opis" => "Air Order", "nazwa" => "air_order_type", "typ" => "lista", "opcje" => $this->SOIE, 'domyslna' => '---- wszystkie ----');
				...
				if (isset($_SESSION['Filtry'][$Pole])) {
                                    ...
                                    if($this->Filtry[$i]['typ'] == "lista"){
                                        ...
                                        if($Pole == "awb"){
                                            ...
                                            $SOIKI_BL = $this->Baza->GetValues("SELECT id_zlecenie FROM orderplus_air_orders WHERE (replace(mawb,'-','') LIKE '%$WartoscCheck%' OR replace(hawb,'-','') LIKE '%$WartoscCheck%')");
                                            $SOIKI_BL[] = -1;
                                            if(count($SOIKI_BL) > 0){
        ...
        function DomyslnyWarunek(){
            for ($i = 0; $i < count($this->Filtry); $i++) {
                $Pole = $this->Filtry[$i]['nazwa'];
                if ($Pole == "awb" && isset($_SESSION['Filtry'][$Pole])) {
                    return "";
                }
            }
				...
				if (isset($_SESSION['Filtry'][$Pole])) {
                                    ...
                                    if($this->Filtry[$i]['typ'] == "lista"){
                                        ...
                                        if($Pole == "awb"){
                                            ...
                                            $SOIKI_BL[] = -1;
                                            if(count($SOIKI_BL) > 0){
                                                $Where .=  ($Where != '' ? ' AND ' : '')."air_order_id IN(".implode(",", $SOIKI_BL).")";
                                            }
                                        }else{
                                            $Where .= ($Where != '' ? ' AND ' : '').($AliasTabeli ? "$AliasTabeli." : '')."$Pole LIKE '%$Wartosc%'";
                                        }
        ...
        function DomyslnyWarunekDrogowe(){
            for ($i = 0; $i < count($this->Filtry); $i++) {
                $Pole = $this->Filtry[$i]['nazwa'];
                if ($Pole == "awb" && isset($_SESSION['Filtry'][$Pole])) {
                    return "air_order_id > 0";
                }
            }
            return "air_order_id > 0 AND air_order_id IN(0,".implode(",", $this->SOIEK).")";
        }
	
	function PobierzListeElementow($Filtry = array()) {
                $Wynik = array(
			"numer_zlecenia" => 'Numer zlecenia',
                        "id_zlecenie" => array("naglowek" => 'Numer AO')
		);
		return $Wynik;
	}
		...
		$Akcje = array();
                if($Dane['typek'] == "lotnicze" || $TH){
                    $Akcje[] = array('img' => "podglad_button", 'title' => "Podglad", "akcja_link" => "podglad-morskie-zlec.php?soid={$Dane['id_zlecenie']}&id={$Dane['zlecenie_ao_id']}", "target" => true);
                    $Akcje[] = array('img' => "mail_button", 'title' => "Email", "akcja" => "email");
                    $Akcje[] = array('img' => "copy_button", 'title' => "Kopiuj", "akcja_link" => "?modul=$this->Parametr&akcja=duplikacja&soid={$Dane['id_zlecenie']}&did={$Dane['zlecenie_ao_id']}");

                     $TerminEdycji = date("Y-m-d", strtotime("+1 days"));
                    if ($this->Uzytkownik->IsAdmin() || $this->Dzis < $TerminEdycji){
                        $Akcje[] = array('img' => "edit_button", 'title' => "Edycja", "akcja_link" => "?modul=$this->Parametr&akcja=edycja&soid={$Dane['id_zlecenie']}&id={$Dane['zlecenie_ao_id']}");
                    }else{
                        $Akcje[] = array('img' => "edit_button_grey", 'title' => "Edycja");
                    }
                    if($Dane['id_zlecenie'] == 0){
                        $Akcje[] = array('img' => "add_button", 'title' => "Wystaw SO", "akcja_link" => "?modul=zlecenia_lotnicze&akcja=dodaj_export&zid={$Dane['zlecenie_ao_id']}");
                    }else{
                        $Akcje[] = array();
                    }
                    if($this->Uzytkownik->IsAdmin() || ($_SESSION["uprawnienia_id"] == 2 && $this->Dzis < $TerminEdycji)){
                        $Akcje[] = array('img' => "delete_button", 'title' => "Kasowanie", "akcja" => "?modul=$this->Parametr&akcja=kasowanie&soid={$Dane['id_zlecenie']}&id={$Dane['zlecenie_ao_id']}");
                    }else{
                ...
                if($Dane['typek'] == "drogowe"){
                    ...
                    $Akcje[] = array('img' => "mail_button", 'title' => "Email", "akcja_link" => "?modul=zlecenia&akcja=email&id={$Dane['id_zlecenie']}&ret=air");
                    $Akcje[] = array('img' => "copy_button", 'title' => "Kopiuj", "akcja_link" => "?modul=zlecenia&akcja=duplikacja&id={$Dane['id_zlecenie']}&ret=air");
                    $TerminEdycji = date("Y-m-d", strtotime($Dane['termin_rozladunku']."+3 days"));
                    if ($this->Uzytkownik->IsAdmin() || $this->Dzis < $TerminEdycji){
                        $Akcje[] = array('img' => "edit_button", 'title' => "Edycja", "akcja_link" => "?modul=tabela_rozliczen_nowa&akcja=edycja&id={$Dane['id_zlecenie']}&ret=air");
                    }else{
                        $Akcje[] = array('img' => "edit_button_grey", 'title' => "Edycja");
                    }
                    if($Dane['id_zlecenie'] == 0){
                    ...
                    if($this->Uzytkownik->IsAdmin() || ($_SESSION["uprawnienia_id"] == 2 && $this->Dzis < $TerminEdycji)){
                        $Akcje[] = array('img' => "delete_button", 'title' => "Kasowanie", "akcja" => "?modul=zlecenie&akcja=kasowanie&id={$Dane['id_zlecenie']}&ret=air");
                    }else{
                        $Akcje[] = array('img' => "delete_button_grey", 'title' => "Kasowanie");
                    }
                }
        ...
        function AkcjaDodawanie(){
            ...
            if($SOID > 0){
                $SOI = $this->Baza->GetData("SELECT * FROM orderplus_air_orders WHERE id_zlecenie = '$SOID'");
            }else{
                $SOI = array('air_order_type' => "E");
            }
            if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['AirZlec'])){ 
                if($this->Zapisz($SOID, $SOI)){
                    $this->ShowOK();
                }else{
                    $Values = $_POST['AirZlec'];
                    if($SOID > 0){
                ...
                if($SOI['air_order_type'] == "I"){
                    ...
                    include(SCIEZKA_SZABLONOW."forms/zlecenie-lotnicze-form-import.tpl.php");
                }else{
                    $Values['miejsce_podjecia'] = $SOI['depot'];
                    $Values['terminal'] = $SOI['terminal'];
                    $Values['nr_booking'] = $SOI['booking_no'];
                    $Values['carrier'] = $SOI['carrier'];
                    $Values['feeder'] = $SOI['feeder'];
                    $Values['pod'] = $SOI['pod'];
                    include(SCIEZKA_SZABLONOW."forms/zlecenie-lotnicze-form-export.tpl.php");
                }
            }
        }
        ...
        function AkcjaDuplikacja($ID){
            ...
            if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['AirZlec'])){
                ...
                $Values = $this->Baza->GetData("SELECT * FROM orderplus_air_orders_zlecenia WHERE zlecenie_ao_id = '{$_GET['did']}'");
                $Values['cont_number'] = array();
                if($SOID > 0){
                    $Wykorzystane = $this->GetUsed($SOID, $_GET['did']);
                    $Values = $this->GetContainers($Values, $SOID);
                    ...
                    $SOI = array();
                    $SOI['mode'] = "FCL";
                    $Values['FCL'] = $this->Baza->GetRows("SELECT * FROM orderplus_air_orders_fcl WHERE id_zlecenie = '$SOID' AND no_so_id = '{$_GET['did']}'");
                }
                if($SOI['air_order_type'] == "I"){
                    include(SCIEZKA_SZABLONOW."forms/zlecenie-form-import.tpl.php");
                }else{
        ...
        function AkcjaEdycja($ID){
            ...
            if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['AirZlec'])){
                ...
                $Values = $this->Baza->GetData("SELECT * FROM orderplus_air_orders_zlecenia WHERE zlecenie_ao_id = '$ID'");
                $Values['cont_number'] = $this->Baza->GetValues("SELECT cont_number FROM orderplus_air_orders_zlecenia_fcl WHERE zlecenie_ao_id = '$ID'");
                if($SOID > 0){
                    $Wykorzystane = $this->GetUsed($SOID, $ID);
                    $Values = $this->GetContainers($Values, $SOID);
                }else{
                    $SOI = array();
                    $SOI['mode'] = "FCL";
                    $this->Baza->Query("SELECT * FROM orderplus_air_orders_fcl WHERE id_zlecenie = '$SOID' AND no_so_id = '$ID'");
            ...
            if((isset($SaveValues['cont_number']) && count($SaveValues['cont_number']) > 0) || ($SOID == 0 && count($SaveValues['FCL'] > 0))){
                ...
                if($SOID > 0 || !$CheckedFCL){
                    if(isset($_GET['id']) && is_numeric($_GET['id'])){
                        ...
                        $NumerZlecenia = $this->Baza->GetValue("SELECT numer_zlecenia FROM orderplus_air_orders_zlecenia WHERE zlecenie_ao_id = '{$_GET['id']}'");
                        $NumerExp = explode("/", $NumerZlecenia);
                        $SaveValues['numer_zlecenia'] = "{$NumerExp[0]}/$identyfikator_przewoznika/{$NumerExp[2]}/{$NumerExp[3]}/SO";
                        $Zapytanie = $this->Baza->PrepareUpdate("orderplus_air_orders_zlecenia", $SaveValues, array('zlecenie_ao_id' => $_GET['id']));
                    }else{
                        $SaveValues['id_uzytkownik'] = $_SESSION['id_uzytkownik'];
                        $identyfikator_przewoznika = $this->Baza->GetValue("SELECT identyfikator FROM orderplus_przewoznik WHERE id_przewoznik = '{$SaveValues['id_przewoznik_to']}'");
                        $SaveValues['numer_krotki'] = intval($this->Baza->GetValue("SELECT max(soz.numer_krotki) FROM orderplus_air_orders_zlecenia soz LEFT JOIN orderplus_air_orders so ON(so.id_zlecenie = soz.id_zlecenie) WHERE so.air_order_type = '{$SOI['air_order_type']}' OR so.air_order_type IS NULL")) + 1;
                        $KoniecNr = date("m/Y");
                        $SaveValues['numer_zlecenia'] = "{$SaveValues['numer_krotki']}/$identyfikator_przewoznika/$KoniecNr/SO";
                        $Zapytanie = $this->Baza->PrepareInsert("orderplus_air_orders_zlecenia", $SaveValues);
                    }
                    if($this->Baza->Query($Zapytanie)){
                        ...
                        $this->ID = $ID;
                        if($SOID == 0){
                            $this->Baza->Query("DELETE FROM orderplus_air_orders_fcl WHERE id_zlecenie = '0' AND no_so_id = '$ID'");
                            $UsedConty = array();
                            foreach($Conty as $Cont){
                                ...
                                $ZapFCL = $this->Baza->PrepareInsert("orderplus_air_orders_fcl", $Cont);
                                $this->Baza->Query($ZapFCL);
                                echo mysql_error();
                                $UsedConty[] = $Cont['cont_no'];
                            }
                        }
                        $this->Baza->Query("DELETE FROM orderplus_air_orders_zlecenia_fcl WHERE zlecenie_ao_id = '$ID'");
                        foreach($UsedConty as $ContNo){
                            $FCLSave['cont_number'] = $ContNo;
                            $FCLSave['zlecenie_ao_id'] = $ID;
                            $Zap2 = $this->Baza->PrepareInsert("orderplus_air_orders_zlecenia_fcl", $FCLSave);
       ...
       function ValidateContNumber($Conts){
        ...
        function GetUsed($SOID, $ID){
            $Wykorzystane = array();
            $Orders = $this->Baza->GetValues("SELECT zlecenie_ao_id FROM orderplus_air_orders_zlecenia WHERE id_zlecenie = '$SOID' AND zlecenie_ao_id != '$ID'");
            if($Orders){
                $Wykorzystane = $this->Baza->GetValues("SELECT cont_number FROM orderplus_air_orders_zlecenia_fcl WHERE zlecenie_ao_id IN(".implode(",",$Orders).")");
            }
            return $Wykorzystane;
        }
        ...
        function AkcjaDrukuj($ID, $Akcja){
            if($this->SprawdzUprawnienie("zlecenia_lotnicze_zlec") || isset($_GET['hash'])){
                if($Akcja == "podglad" || $Akcja == "podglad_przewoznik"){
                    ...
                    if($soid == 0){
                        $SeaOrder = array('mode' => 'FCL', 'air_order_type' => "E");
                    }else{
                        $SeaOrder = $this->Baza->GetData("SELECT * FROM orderplus_air_orders WHERE id_zlecenie = '$soid'");
                    }
                    ...
                    if($SeaOrder['mode'] == "FCL"){
                        $Conty = $this->Baza->GetValues("SELECT cont_number FROM orderplus_air_orders_zlecenia_fcl WHERE zlecenie_ao_id = '$ID'");
                        if(count($Conty) > 0){
                            $Containers = $this->Baza->GetRows("SELECT * FROM orderplus_air_orders_fcl WHERE id_zlecenie = '$soid'");
                        }
                    }else{
                        $Conty = GetValues("SELECT cont_number FROM orderplus_air_orders_zlecenia_lcl WHERE zlecenie_ao_id = '$ID'");
                        if(count($Conty) > 0){
                            $Containers = $this->Baza->GetRows("SELECT * FROM orderplus_air_orders_lcl WHERE id_zlecenie = '$soid'");
                        }
                    }
        ...
        function ShowRecord($Element, $Nazwa, $Styl){
            if($Nazwa == "conty"){
                $Conty = $this->Baza->GetValues("SELECT cont_number FROM orderplus_air_orders_zlecenia_fcl WHERE zlecenie_ao_id = '{$Element['zlecenie_ao_id']}'");
                echo("<td$Styl>".implode("<br />",$Conty)."</td>");
            }else if($Nazwa == "id_zlecenie"){
                if($Element[$Nazwa] == 0){
                    $Numer = "Bez SO";
                }else{
                    $Numer = $this->Baza->GetValue("SELECT numer_zlecenia FROM orderplus_air_orders WHERE id_zlecenie = '{$Element[$Nazwa]}'");
                }
